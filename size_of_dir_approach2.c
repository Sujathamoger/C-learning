    ...
    while ((dirData = readdir(poDir)))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //printf("general_getFolderSize: Item size (file) (%s): %d, %d:%d\n",fullPath,(int)buffer.st_size, (int)buffer.st_blksize, (int)buffer.st_blocks);
            }
        }
    }
...
int main()
{
	...
	char *dirPath= "/ws/ssanger/test_code/libreplaceFunction";

	if (general_getFolderSize(dirPath,&folderSize) == EXIT_FAILURE)
	{
		printf("Error reading folder size!");
	}
	else
	{
		printf(" %s : Folder size: %d\n", dirPath, folderSize);
	}
	return EXIT_SUCCESS;
}
